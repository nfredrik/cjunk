...
int set_hex_format(const config_setting_t *list, int i) {

config_setting_t *int_settings;

      if (NULL == (int_settings = config_setting_get_elem(list, i))) {
        puts("Wrong getting");
        return -1;
      }
  

      if (!(config_setting_set_format(int_settings, CONFIG_FORMAT_HEX))) {
        puts("Failed setting format");
        return -1;   
      }
...
int main(int argc, char **argv)
{
  ...
  if(array)
  {
    for(i = 0; i < config_setting_length(array); i++)
    {
      ...
      if(set_hex_format(array, i)) {
        puts("Wrong getting settings for int");  
      }
#if 0
      if (NULL == (int_settings = config_setting_get_elem(array, i)))
        puts("Wrong getting settings for int");  
...
#endif
//      if (NULL == config_setting_set_int_elem(array, i, val))
//        puts("Wrong set int in array");         
      // New ends NEW CODE
    }
  }
